...
\subsection{ERROR AND ERROR BOUND}
    The error of a network solution $u$ is defined as $ \Err := u - v$.
    We are interested in \textit{bounding} the error with a scalar function $\Bound$ such that $\|\Err(t)\| \leq \Bound(t)$ or $\|\Err(x, y)\| \leq \Bound(x, y)$ where $\|\Err\| = \|u - v\|$ is the \textit{absolute error}.
...
\subsubsection{Single Linear ODE with Constant Coefficients}\label{section:single-linear-ode-with-constant-coefficients}
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}
            ...
            \Ensure $\left|\Err(t_k)\right| \leq \Bound(t_k)$ for all $k$. 
            % \vspace{0.5em}
...
\subsubsection{System of Linear ODEs with Constant Coefficients} \label{section:system-of-linear-odes-with-constant-coefficients}
    ...
    An \textit{elementwise bound} (vector) $\pmb{\Bound}(t)$ can be formulated as 
    {
        \small
        \begin{equation}
            \pmb{\Err}^{\abs}(t) \preceq \pmb{\Bound}(t) := P^{\abs} (P^{-1})^{\abs} \ \vect{I}\left[\vect{r}^{\abs}(t)\right],
        \end{equation}
    }
    where superscript $\abs$ denotes elementwise absolute value and symbol $\preceq$ denotes elementwise inequality. In the meantime, a \textit{norm bound} (scalar) $\Bound(t)$ also exists
    {
        \small
        \begin{equation}
            \left\|\pmb{\Err}(t)\right\| \leq \Bound(t) := \mathrm{cond}(P)\left\|\vect{I}\Big[\|\vect{r}(t)\|\vect{1}\Big]\right\|
        \end{equation}
    }
...
\subsection{SINGLE LINEAR ODE WITH CONSTANT COEFFICIENTS}
    ...
    We pick $f(t) =2t^2+8t+7$, $g(t) = t^2+t+3$, and $h(t)=1-2t$, so that the manufactured solution is $v(t) = t^2 + t + 1$.
    See Fig. \ref{fig:2nd-order-bound} for visualization of the loose and tight bounds.
...
\subsection{LINEAR ODE SYSTEM WITH CONSTANT COEFFICIENTS} \label{section:high-dimension}
    ...
    \begin{figure}[h]
        ...
        \caption{\smaller \textit{Componentwise} bound (upper) and \textit{norm} bound (lower) for linear ODE system with constant coefficients}\label{fig:system-bound}
    \end{figure}
...
\subsection{NONLINEAR ODE -- DUFFING EQUATION} \label{section:experiment-duffing}
    ...
    \begin{figure}
        ...
        \vspace{-2em}
        \caption{\smaller RKF45 and Network Solutions (max-degree 0$\sim$6) to Duffing equation \ref{eq:duffing} for $\varepsilon \in (-0.9, 0.9)$}\label{fig:duffing-solution}
        \vspace{0.5em}
        \includegraphics[width=\linewidth]{assets/duffing-error.pdf}
        \caption{\smaller True Error vs. error bound (max-degree 0$\sim$6) of neural network solution to Duffing Equation \ref{eq:duffing} for $\varepsilon \in (-0.9, 0.9)$}\label{fig:duffing-error}
    \end{figure}
...
\subsubsection{PDE Error Bound Evaluation Using Alg. \ref{alg:linear-first-order-pde-general}}
    ...
    \begin{figure}[!h]
        \centering
        \subfloat[\centering \smaller $\mathcal{C}: \begin{cases*} x' = -x - y \\ y' = x - y \end{cases*}$]{{\includegraphics[width=0.50\linewidth]{assets/characteristics.pdf}}}
        \subfloat[\centering \smaller $\mathcal{C}: \begin{cases*} x' = x^2 + y^2 + 1 \\ y' = x^2 - y^2 + 2 \end{cases*}$]{{\includegraphics[width=0.49\linewidth]{assets/unsolvable-characteristics.pdf}}}
        \caption{
            \smaller
...
\subsubsection{PDE Error Bound Evaluation Using Alg. \ref{alg:linear-first-order-pde-constant}}
    ...
    \vspace{-0.8em}
    The characteristic curves {\small $\mathcal{C}: \begin{cases*} x'(s) = x^2+y^2+1 \\[-0.25em] y'(s) = x^2 - y^2 + 2 \end{cases*}$} are given by a nonlinear ODE, which are hard to solve analytically. See Fig. \ref{fig:characteristics}b for visualization.
    ...
    \begin{figure}[!h]
        ...
        \caption{
            \smaller
            Constant bound $B$ and max absolute error over domain at different epochs of training.
