...
\begin{abstract}
    ...
    However, a major criticism lies in the lack of error bounds for obtained solutions.
    ...
    The technique evaluates the error bound in an efficient manner, and has certain variants that provide tighter bounds at the cost of longer run time.
\end{abstract}
...
\section{Introduction}
    ...
    Section \ref{section:error-bound-for-ode} and \ref{section:error-bound-for-pde} propose various algorithms for error bound of ODEs and PDEs respectively.
...
\section{EXISTING WORK}\label{section:existing-work}
    ...
    \cite{de2022errorhyperbolic} derives an error bound for ReLU networks on parametric hyperbolic conservation laws.
    \cite{de2022errorkolmogorov} shows that there exists PINNs with arbitrarily small residual for Kolmogorov PDEs.
...
\section{ERROR BOUND FOR PDE} \label{section:error-bound-for-pde}
    ...
    Hence, the original PDE can be reformulated as an ODE
    {
        \small
        \begin{equation}
            v'(s) + c\big(x(s), y(s)\big)v(s) = f(s) \quad \text{s.t. } v(0) = g(x_0, y_0)
        \end{equation}
    }
...
\section{RELEVANT EXPERIMENTS}\label{section:experiments}
    ...
    Throughout this section, we always use networks with 2 hidden layers each consisting of 32 hidden units and a single output unit.
...
\subsection{LINEAR ODE SYSTEM WITH CONSTANT COEFFICIENTS} \label{section:high-dimension}
    ...
    After obtaining the residual information {\small $\vect{r}(t)=\frac{\mathrm d}{\mathrm d t}\vect{u}(t) + A\vect{u}(t) - \vect{f}(t)$}, we apply Alg. \ref{alg:system-bound} to obtain componentwise bound and norm bound of {\small $\pmb{\Err} = \vect{u}-\vect{v}$}. See Fig. \ref{fig:system-bound}. It is shown in Fig. \ref{fig:system-bound} that the bounds hold true over the domain.
...
\subsubsection{PDE Error Bound Evaluation Using Alg. \ref{alg:linear-first-order-pde-general}}
    ...
    in spatial domain $\Omega=[-1, 1]^2$, subject to the boundary constraints of $v(x, \pm1) =2x\pm 3$ and $v(\pm 1, y) = 3y \pm 2$. 
    The exact (manufactured) solution is given by $v(x, y) = 2x + 3y$.
    The characteristic curves are integral curves {\small $\mathcal{C}: \begin{cases*} x'(s) = -x - y \\[-0.25em] y'(s) = x - y \end{cases*}$}, which is solved by {\small $\begin{cases*} x(s) = R_0 e^{-s} \cos (s+\theta_0)\\[-0.25em] y(s) = R_0 e^{-s} \sin(s + \theta_0) \end{cases*}$}, where $R_0 = \sqrt{x_0^2+y_0^2}$ and $\theta_0 = \mathrm{atan2}(y_0, x_0)$ are constants determined by the starting point $(x_0, y_0) \in \Gamma = \partial \Omega$.
