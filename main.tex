...
\subsubsection{System of Linear ODEs with Constant Coefficients} \label{section:system-of-linear-odes-with-constant-coefficients}
    ...
    Let $n_k$ be the size of Jordan block $J_k$, we construct the following operator matrix
    {
        \smaller
        \begingroup 
        \setlength\arraycolsep{1pt}
    ...
    An \textit{elementwise bound} (vector) $\pmb{\Bound}(t)$ can be formulated as 
    {
        \small
        \begin{equation}
            \Err_{\vect{u}}^{\abs} \preceq \pmb{\Bound}(t) := P^{\abs} (P^{-1})^{\abs} \ \vect{I}\left[\vect{r}^{\abs}(t)\right],
        \end{equation}
    }
    where superscript $\abs$ denotes elementwise absolute value and symbol $\preceq$ denotes elementwise inequality. In the meantime, a \textit{norm bound} (scalar) $\Bound(t)$ also exists
    {
        \small
        \begin{equation}
            \left\|\Err_{\vect{u}}(t)\right\| \leq \Bound(t) := \mathrm{cond}(P)\left\|\vect{I}[\vect{1}]\right\| \, \|\vect{r}(t)\|,
        \end{equation}
    }
    where $\mathrm{cond}(P)$ is the conditional number of $P$ w.r.t. induced matrix norm and $\vect{1}$ is an $n\times 1$ column vector of $1$s. (Proof in supp. material) See Alg. \ref{alg:system-bound} for implementation.
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}
            ...
            \State $\left\{\Bound(t_\ell)\right\}_{\ell=1}^{L} \gets \{\mathrm{cond}(P)\left\|\vect{I}[\vect{1}]\right\| \, \|\vect{r}(t_\ell)\|\}_{\ell=1}^{L}$
